...
def scan_domain(domain):
    try:
        ...
        return a_records, ns_records, ping_results, result_a.stdout.strip().split('\n') + result_ns.stdout.strip().split('\n')
...
def write_results_to_excel(domains, a_records, ns_records, ping_results, results, output_file):
    workbook = openpyxl.Workbook()
    sheet = workbook.active
    sheet.title = 'DNS Scan Results'

    # Add header row with dynamic columns for A Record, Ping Result, and multiple NS Records
    header = ['Domain', 'A Record', 'Ping Result']
    max_ns_records = max(len(ns) for ns in ns_records)
    header.extend([f'NS Record {i+1}' for i in range(max_ns_records)])
    sheet.append(header)

    # Write data rows
    for domain, a_record, ping_result, ns_record, result in zip(domains, a_records, ping_results, ns_records, results):
        row = [domain, ', '.join(a_record), ', '.join(ping_result)]  # A record may have multiple IPs
...
def main(input_file, output_file):
    ...
    for domain in domains:
        a_record, ns_record, ping_result, result = scan_domain(domain)
        a_records.append(a_record)
        ns_records.append(ns_record)
        ping_results.append(ping_result)
        results.append(result)

    write_results_to_excel(domains, a_records, ns_records, ping_results, results, output_file)
